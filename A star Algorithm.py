#!/usr/bin/env python
# coding: utf-8

# In[1]:


import os
import time
import math
import random
import pygame
from pygame.locals import *
import heapq
import tkinter as tk


pygame.init()
pygame.mixer.init()


SCREEN_WIDTH = 800
SCREEN_HEIGHT = 600


WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
RED = (255, 0, 0)
BLUE = (0, 0, 255)
YELLOW = (255, 255, 0)


explosion_sound = pygame.mixer.Sound("explosion.wav")
laser_sound = pygame.mixer.Sound("laser.wav")
hyperspace_sound = pygame.mixer.Sound("hyperspace.wav")

font_path = os.path.join("fonts", r"C:\Users\ACER\spacewar game\BruceForeverRegular-X3jd2.ttf")  

custom_font = pygame.font.Font(font_path, 26)


screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
pygame.display.set_caption("Space War")


player_img = pygame.image.load("player.svg")
enemy_img = pygame.image.load("enemy.svg")
bullet_img = pygame.image.load("bullet2.svg")
background_img = pygame.image.load("starfield.gif")



class Node:
    def __init__(self, x, y, walkable):
        self.x = x
        self.y = y
        self.walkable = walkable
        self.g_score = float('inf')
        self.h_score = 0
        self.f_score = float('inf')
        self.parent = None

        
        
############################

class Sprite(pygame.sprite.Sprite):
    def __init__(self, image, x, y):
        super().__init__()
        self.image = image
        self.rect = self.image.get_rect(center=(x, y))

    def move(self, dx, dy):
        self.rect.x += dx
        self.rect.y += dy

    def draw(self, screen):
        screen.blit(self.image, self.rect)
        
#######################################################################

class Player(Sprite):
    def __init__(self, image, x, y):
        super().__init__(pygame.transform.scale(image, (50, 50)), x, y)
        self.speed = 0
        self.score = 0
        self.lives = 3
        self.angle = 0
        self.original_image = pygame.transform.scale(image, (50, 50))

    def turn_left(self):
        self.angle += 45
        self.rotate_player()

    def turn_right(self):
        self.angle -= 45
        self.rotate_player()

    def accelerate(self):
        self.speed += 1

    def decelerate(self):
        self.speed -= 1

    def hyperspace(self):
       
        self.rect.x = random.randint(0, SCREEN_WIDTH)
        self.rect.y = random.randint(0, SCREEN_HEIGHT)
        self.speed *= 0.5

    def rotate_player(self):
        self.image = pygame.transform.rotate(self.original_image, self.angle)

    def get_angle(self):
        return self.angle

###############
class Enemy(Sprite):
    def __init__(self, image, x, y):
        super().__init__(pygame.transform.scale(image, (50, 50)), x, y)
        self.speed = 2
        self.graph = create_graph(GAME_WIDTH // 20, GAME_HEIGHT // 20)
        self.path = []
        self.path_index = 0
        self.shoot_timer = 0
        self.shoot_interval = 60
        self.angle = 0
        self.player_position = None

    def update(self, player_position):
        if self.player_position != player_position:  # Player's position has changed
            self.player_position = player_position
            self.find_player_path()

        if self.path:
            next_node = self.path[self.path_index]
            next_x = next_node[0] * 20 + 10
            next_y = next_node[1] * 20 + 10
            dx = next_x - self.rect.centerx
            dy = next_y - self.rect.centery
            distance = math.sqrt(dx ** 2 + dy ** 2)
            if distance < self.speed:
                self.rect.center = (next_x, next_y)
                self.path_index += 1
                if self.path_index >= len(self.path):
                    self.path = []
            else:
                angle = math.degrees(math.atan2(dy, dx))
                self.rect.x += self.speed * math.cos(math.radians(angle))
                self.rect.y += self.speed * math.sin(math.radians(angle))

        

    def find_player_path(self):
        if self.player_position:
            start_node = (self.rect.centerx // 20, self.rect.centery // 20)
            goal_node = (self.player_position[0] // 20, self.player_position[1] // 20)
            self.path = astar_pathfinding(start_node, goal_node, self.graph)
            self.path_index = 0


    


################


class Bullet(Sprite):
    def __init__(self, image, x, y):
        super().__init__(pygame.transform.scale(image, (10, 30)), x, y)
        self.speed = 20
        self.angle = 0

    def update(self):
       
        self.rect.x += self.speed * -math.sin(math.radians(self.angle))
        self.rect.y += self.speed * -math.cos(math.radians(self.angle))
       
        self.rect.width = 10
        self.rect.height = 30
    def rotate_bullet(self, player_angle):
        self.angle = player_angle
            
        
#########################
class Game():
    def __init__(self):
        self.state = "splash"
        self.clock = pygame.time.Clock()
        self.level = 0
        self.lives = 3
        self.score = 0

   
    def show_status(self):
        #"""Displays the current game status."""
        font = pygame.font.Font(None, 24)
        level_text = custom_font.render(f"Level: {self.level}", True, WHITE)
        lives_text = custom_font.render(f"Lives: {self.lives}", True, WHITE)
        score_text = custom_font.render(f"Score: {self.score}", True, WHITE)
        screen.blit(level_text, (30, 10))
        screen.blit(lives_text, (300, 10))
        screen.blit(score_text, (570, 10))

##################################

#  dimensions of the game area
GAME_WIDTH = 600
GAME_HEIGHT = 500

def draw_game_interface():
    
    # Draw game area border
        pygame.draw.rect(screen, WHITE, (100, 70, GAME_WIDTH, GAME_HEIGHT), 3)
        hyperspace_sound.play()
    
    
###################################

def create_graph(width, height):
    graph = {}
    cell_size = 20  
    for x in range(width):
        for y in range(height):
            neighbors = []
            
            for dx in [-1, 0, 1]:
                for dy in [-1, 0, 1]:
                    if dx != 0 or dy != 0:  
                        neighbor_x = x + dx
                        neighbor_y = y + dy
                        if 0 <= neighbor_x < width and 0 <= neighbor_y < height:
                            neighbors.append((neighbor_x, neighbor_y))
                            
            graph[(x, y)] = neighbors
    return graph
###########################################################

def astar_pathfinding(start, goal, graph):
    
    distances = {node: float('inf') for node in graph}
    previous = {node: None for node in graph}
    distances[start] = 0

 
    unvisited = [(0, start)]

    while unvisited:
        
        current_distance, current_node = heapq.heappop(unvisited)

        if current_node == goal:
            break

        
        for neighbor in graph[current_node]:
            
            distance = current_distance + 1 

            if distance < distances[neighbor]:
               
                distances[neighbor] = distance
                previous[neighbor] = current_node
             
                heuristic = abs(neighbor[0] - goal[0]) + abs(neighbor[1] - goal[1])
                heapq.heappush(unvisited, (distance + heuristic, neighbor))

    # Reconstruct shortest path
    path = []
    while previous[current_node] is not None:
        path.append(current_node)
        current_node = previous[current_node]
    path.append(start)
    path.reverse()

    return path

#############################
def game_over_screen(score):
    screen.fill((0, 0, 0))
    font_large = pygame.font.Font(font_path, 48)
    font_small = pygame.font.Font(font_path, 24)
    game_over_text = font_large.render("Game Over", True, RED)
    score_text = font_small.render(f"Score: {score}", True, YELLOW)
    restart_text = font_small.render("Press R to restart", True, WHITE)
    quit_text = font_small.render("Press Q to quit", True, WHITE)

    game_over_x = SCREEN_WIDTH // 2 - game_over_text.get_width() // 2
    game_over_y = SCREEN_HEIGHT // 4 - game_over_text.get_height() // 2
    
    score_x = SCREEN_WIDTH // 2 - score_text.get_width() // 2
    score_y = game_over_y + game_over_text.get_height() + 20
    
    restart_x = SCREEN_WIDTH // 2 - restart_text.get_width() // 2
    restart_y = SCREEN_HEIGHT // 2 - restart_text.get_height() // 2
    
    quit_x = SCREEN_WIDTH // 2 - quit_text.get_width() // 2
    quit_y = SCREEN_HEIGHT // 2 + 2 * restart_text.get_height()

    screen.blit(game_over_text, (game_over_x, game_over_y))
    screen.blit(score_text, (score_x, score_y))
    screen.blit(restart_text, (restart_x, restart_y))
    screen.blit(quit_text, (quit_x, quit_y))

    pygame.display.flip()

    while True:
        for event in pygame.event.get():
            if event.type == QUIT:
                pygame.quit()
                exit()
            elif event.type == KEYDOWN:
                if event.key == K_r:
                    return True
                elif event.key == K_q:
                    pygame.quit()
                    exit()

########################################3


game = Game()


draw_game_interface()


game.show_status()


player = Player(player_img, GAME_WIDTH, GAME_HEIGHT)


enemies = pygame.sprite.Group()
for _ in range(7):
    enemy = Enemy(enemy_img, random.randint(0, GAME_WIDTH-50), random.randint(0, GAME_HEIGHT-50))
    enemies.add(enemy)


bullets = pygame.sprite.Group()





#################################################################################################################
##################################################################################################################
def main():

    game = Game()
    player = Player(player_img, GAME_WIDTH, GAME_HEIGHT)


    enemies = pygame.sprite.Group()
    for _ in range(7):
        enemy = Enemy(enemy_img, random.randint(0, GAME_WIDTH-50), random.randint(0, GAME_HEIGHT-50))
        enemies.add(enemy)


    bullets = pygame.sprite.Group()
    
    level_thresholds = [1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000]
    max_level = len(level_thresholds)    
    running = True
    game_over = False
    
    pressed_keys = set()

    while running and not game_over: 
        screen.fill(BLACK)  

        for event in pygame.event.get():
            if event.type == QUIT:
                running = False
            elif event.type == KEYDOWN:
                if event.key == K_LEFT:
                    player.turn_left()
                elif event.key == K_RIGHT:
                    player.turn_right()
                elif event.key == K_UP:
                    player.accelerate()
                elif event.key == K_DOWN:
                    player.decelerate()
                elif event.key == K_SPACE:
                    bullet = Bullet(bullet_img, player.rect.centerx, player.rect.centery)
                    bullet.angle = player.angle
                    explosion_sound.play()
                    bullets.add(bullet)
                   

        
        player.rotate_player()
        player.move(player.speed * -math.sin(math.radians(player.angle)), player.speed * -math.cos(math.radians(player.angle)))

         
        for enemy in enemies:
            enemy.update((player.rect.centerx, player.rect.centery))  


       
        bullets.update()

#################Collision detection between player and game borders
        if player.rect.left < 0:
            player.rect.left = 0
        elif player.rect.right > GAME_WIDTH:
            player.rect.right = GAME_WIDTH
        if player.rect.top < 0:
            player.rect.top = 0
        elif player.rect.bottom > GAME_HEIGHT:
            player.rect.bottom = GAME_HEIGHT
                
                
#############Collision detection between bullets and enemies
        enemy_hit = pygame.sprite.groupcollide(enemies, bullets, True, True)
        for enemy in enemy_hit:
           
            new_enemy = Enemy(enemy_img, random.randint(0, GAME_WIDTH), random.randint(0, GAME_HEIGHT))
            enemies.add(new_enemy)
            
            game.score += 100
        for level, threshold in enumerate(level_thresholds):
            if game.score >= threshold:
                game.level = level + 1 
                break
       
           
################# Collision detection between enemies and player
        enemy_player_hit = pygame.sprite.spritecollide(player, enemies, True)
        if enemy_player_hit:
            game.lives -= 1
            explosion_sound.play()
            if game.lives <= 0:
                game_over = True  

        
        screen.blit(background_img, (0, 0))
        player.draw(screen)
        for enemy in enemies:
            enemy.draw(screen)
        bullets.draw(screen)

       
        draw_game_interface()
        game.show_status()

        pygame.display.flip()
        game.clock.tick(60)
##################################################
        if game_over:
            restart = game_over_screen(game.score)
            if restart:
                game_over = False
               
                player.rect.x = GAME_WIDTH
                player.rect.y = GAME_HEIGHT
                player.speed = 0
                player.score = 0
                player.lives = 3
                player.angle = 0
                enemies.empty()
                game = Game()
                draw_game_interface()
                game.show_status()
                player = Player(player_img, GAME_WIDTH, GAME_HEIGHT)
                enemies = pygame.sprite.Group()
                for _ in range(7):
                    enemy = Enemy(enemy_img, random.randint(0, GAME_WIDTH-50), random.randint(0, GAME_HEIGHT-50))
                    enemies.add(enemy)
                bullets.empty()

    pygame.quit()


if __name__ == "__main__":
    main()


# In[ ]:




